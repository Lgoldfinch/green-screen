...
import com.green.screen.analytics.engine.domain.{BankPrefix, CreateAccountAccessConsentsRequest, UserOpenApiDataDB, UserOpenApiDataUuid, UserUuid}
...
final class CreateAccountAccessConsent[F[_]: MonadThrow: Logger: GenUUID](
  ...
  def run(request: CreateAccountAccessConsentsRequest, bankPrefix: BankPrefix, userUuid: UserUuid): F[Unit] =
    for {
      ...
      _ <- ApplicativeThrow[F].raiseWhen(response.status != AWAU)(new RuntimeException(s"Account access consent status should be AWAU, but instead got ${response.status} for User: $userUuid"))
      ...
      _ <- Logger[F].info(s"Account access consent awaiting authorisation for Consent id: ${response.consentId} and User: $userUuid")
    } yield ()
