...
import com.green.screen.analytics.engine.domain.{CreateAccountAccessConsentsRequest, CreateAccountAccessConsentsRequestData, Permission, UserUuid}
...
object UserRoutes:
  def routes[F[_]: Concurrent: Logger](userScores: GetUserScores[F], createAccountAccessConsent: CreateAccountAccessConsent[F]): AuthedRoutes[UserType, F] = {
